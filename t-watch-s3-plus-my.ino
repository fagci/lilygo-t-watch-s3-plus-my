...
enum {
    ...
    SCR_RECON, SCR_BATTERY, SCR_BLE, SCR_NOTIF
};
...
void loop()
{
    ...
    if (state::display != Display::OFF) {
        if (!drawerOpen) {              // drawer перекрывает экран и статусбар
            ...
            // Дисплей в LV_DISPLAY_RENDER_MODE_PARTIAL. После оседания перехода
            // один раз инвалидируем весь экран, чтобы LVGL перерисовал и зазоры
            // между лейблами (иначе панель держит пиксели прошлого экрана).
            // NB: lv_refr_now() здесь НЕЛЬЗЯ — он конфликтует с асинхронным
            // DMA-flush драйвера LilyGo и корёжит экран после nav.
            if (tileNeedsRefresh) {
